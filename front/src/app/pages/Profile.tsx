...
import { currentUser, MOCK_ALBUMS, MOCK_DIARY, MOCK_USERS } from "../data/mock"
...
export function ProfilePage() {
  ...
  const profileUser =
    !userId || userId === currentUser.id
      ? currentUser
      : MOCK_USERS.find((user) => user.id === userId) ?? currentUser
    ...
    <div className="flex-1 p-8 overflow-y-auto w-full max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-12">
        <div className="flex items-center gap-6">
          <div className="relative">
            <Avatar
              ...
              size="xl"
              className="w-32 h-32 border-4 border-[#1a1625]"
            />
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mb-3">
              Entusiasta Musical • Membro desde{" "}
              {profileUser.memberSince ?? "2024"} •{" "}
              <span className="text-[#9b6cf3]">{profileUser.username}</span>
            </p>

            <p className="text-slate-300 italic text-sm">
              {profileUser.bio ?? "Apaixonado por descobrir sons novos e comentar álbuns com sinceridade."}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          {isOwnProfile ? (
            ...
            <Button variant={profileUser.isFollowing ? "outline" : "purple"}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <StatCard
          ...
          value={String(profileUser.followers ?? 124)}
        />
        <StatCard
          label="Seguindo"
          value={String(profileUser.following ?? 45)}
        />
        <StatCard
          label="Horas Catalogadas"
          value={String(profileUser.hoursLogged ?? 380)}
        />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 mb-12">
        ...
        <div className="xl:col-span-1">
          <div className="flex items-center justify-between mb-6">
            ...
            <button className="text-[#9b6cf3] text-sm hover:underline">
              Ver todos
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {MOCK_ALBUMS.slice(0, 4).map((album) => (
              <Link
                to={`/album/${album.id}`}
                key={album.id}
                className="aspect-square rounded-xl overflow-hidden shadow-lg border border-white/5 relative group cursor-pointer block"
              >
                <ImageWithFallback
                  src={album.cover}
                  alt={album.title}
        ...
        <div className="xl:col-span-2 flex flex-col gap-8">
          ...
          <div className="bg-[#1a1625] rounded-2xl p-6 border border-white/5">
            <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
              <span className="w-4 h-4 bg-[#7b3fe4] rounded-sm block" />
              Insights de Audição
            </h2>
              ...
              {MOCK_DIARY.slice(0, 3).map((item) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-white font-medium truncate group-hover:text-[#9b6cf3] transition-colors">
                      {item.album.title}
                    </h4>
      ...
      <div className="bg-[#1a1625] rounded-2xl p-8 border border-white/5 overflow-x-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            <Calendar className="w-5 h-5 text-[#9b6cf3]" />
            Mapa de Calor de Audição
          </h2>
          ...
          {Array.from({ length: 52 }).map((_, weekIdx) => (
            <div key={weekIdx} className="flex flex-col gap-1.5">
              {Array.from({ length: 7 }).map((_, dayIdx) => {
                const activityLevel =
                  ((weekIdx + 1) * (dayIdx + 3) + profileUser.id.length) % 10 / 10
                ...
                return (
                  <div
                    key={`${weekIdx}-${dayIdx}`}
                    className={`w-[14px] h-[14px] rounded-sm ${colorClass}`}
                  />
                )
              })}
            </div>
